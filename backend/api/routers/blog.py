...
async def create(req: schemas.blog.BlogCreate, db: Session = Depends(get_db), current_user=Depends(security.get_current_user)):
...
async def update(id: int, req: schemas.blog.BlogCreate, db: Session = Depends(get_db), current_user=Depends(security.get_current_user)):
